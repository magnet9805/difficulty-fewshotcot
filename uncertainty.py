...
def main():
    ...
    print(f'result 결과는 대충~ : {result}')
    # output the results
    path = f"./uncertainty_result_{args.dataset}_k{args.num_trails}.txt"
...
def generate_uncertainty_qes(args, question):   #불확실한 질문 생성 
    if args.dataset in ("gsm8k", "asdiv", "svamp", "singleeq", "addsub", "multiarith"):               #GSM8K사용
...
#step by step cot작성 
        ...
        if args.method == "zero_shot_cot":
            ...
            prompt_list[0] += responses + args.direct_answer_trigger
...
def create_uncertainty(args, questions):           #불확실성 측정 
    result = []
    count = 0
